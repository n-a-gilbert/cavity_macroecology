...
secondary <- d |> 
  ...
  dplyr::mutate( soft_edge = ifelse(range_dist <= quantile(range_dist, 0.2), "edge", "nonedge")) |> 
...
for( i in 1:length(secondary)){ # loop through non-excavator species
  ...
  focal_df <- d |> 
    ...
    dplyr::mutate( position = ifelse(range_dist <= quantile(range_dist, 0.2), "edge", 
                                     ifelse(range_dist >= quantile(range_dist, 0.8), "core", "junk"))) |> 
