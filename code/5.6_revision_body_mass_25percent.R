...
for( i in 1:length(secondary)){ # loop through non-excavator species
  ...
  n_secondary_0.3 <- d |> 
    ...
    dplyr::filter(mass_ratio >= 0.75 & mass_ratio <= 1.25) |> 
    dplyr::group_by(cell_id) |> 
  ...
  res[[i]] <- focal_df |> 
    ...
    dplyr::left_join(n_secondary_0.3) |> 
    # dplyr::left_join(n_primary) |> 
    dplyr::left_join(n_primary_0.3) |> 
...
df <- d |> 
  ...
  tidyr::pivot_longer(c(n_secondary_0.5, 
                        sr_secondary_0.5, 
                        n_primary_0.5, 
                        sr_primary_0.5) , names_to = "type", values_to = "value") |> 
...
final2 <- df |> 
  dplyr::filter( metric == "n" ) |>  # focus on abundance
  dplyr::filter( group == "primary" ) |>  # non-excavators
  dplyr::filter( mass_ratio == "1.3" ) |>  # all sizes
  dplyr::mutate(x = as.numeric(scale(log1p(value))), # scale the log-transformed summed abundance
                is_edge = factor(is_edge) ) |>    # I guess we still have the core cells
  dplyr::rename(edge = is_edge, 
...
restab <- bind_rows(res) |> 
  cbind(tibble::tibble(
    group = c( "non-excavator", "excavator"),
    size = c("Within 25% body mass", "Within 25% body mass"))) |> 
