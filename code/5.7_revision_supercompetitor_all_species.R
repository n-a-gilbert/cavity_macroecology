...
ggplot() +
  ...
  geom_rect(
    ...
    alpha = 0.3) +
  geom_point(
    data = dplyr::filter(for_plot, sig == "not significant"),
    aes(x = mass_ratio, y = estimate), color = "gray60", size = 2) + 
  geom_point(
    data = dplyr::filter(for_plot, sig == "significant"),
    aes(x = mass_ratio, y = estimate), color = "black", size = 2) +
  ggrepel::geom_text_repel(
    data = dplyr::filter(for_plot, sig == "significant"),
    aes(x = mass_ratio, 
        y = estimate, label = com1), size = 2) +
