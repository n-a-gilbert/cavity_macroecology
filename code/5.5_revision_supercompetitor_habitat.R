...
ghm <- terra::rast(here::here("data/GHM_27km_WGS84.tif"))
...
pts84_vect <- terra::vect( pts84 )

tmp <- extract(ghm, pts84_vect,
               fun = mean, 
               na.rm = TRUE, 
               bind = TRUE)
...
for( i in 1:length(secondary)){
  ...
  other_df <- d |> 
    ...
    dplyr::left_join(masses) |> 
    dplyr::select(cell_id, com2 = com, n2 = n, mass, ghm) |> 
...
for(i in 1:length(ucom1)){
  ...
  tmp <- pw |> 
    ...
    dplyr::filter(!is.na(ghm)) |> 
    dplyr::mutate(x = as.numeric(scale(log1p(n2))),
                  ghm = as.numeric(scale(ghm)))
  
  m <- glmmTMB(
    n1 ~ 1 + x + ghm, # abundance of focal non-excavator (response) predicted by abundance of either starling or hosp
  ...
  # stash results
  glmm_output[[i]] <- ms$coefficients$cond[2:3,] |> 
...
ggplot() +
  ...
  geom_rect(
    ...
    alpha = 0.3) +
  geom_point(
    data = dplyr::filter(for_plot,
                         sig == "not significant"),
    aes(x = mass_ratio, y = estimate),
